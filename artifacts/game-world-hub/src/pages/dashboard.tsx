                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {friendsSummary?.friends.map(entry => {
                    ...
                    return (
                      <div
                        key={entry.id}
                        className="group relative flex flex-col border border-border bg-card hover:border-primary/30 transition-all duration-200"
                        style={
                      ...
                      >
                        <Link href={`/profile/${f.id}`} className="flex flex-col flex-1">
                          {/* banner / bg — fixed height, no overflow issue */}
                              ...
                              <div
                                ...
                                style={{
                                  background: (f as any).profileFrameColor
                                    ? `linear-gradient(135deg, ${(f as any).profileFrameColor}33 0%, ${(f as any).profileFrameColor}08 100%)`
                        ...
                        {isConfirmingBlock ? (
                          ...
                          <div className="flex border-t border-border">
                            <button
                              className="flex-1 flex items-center justify-center gap-1 py-2 text-muted-foreground hover:text-primary hover:bg-primary/5 transition-colors disabled:opacity-30 disabled:cursor-not-allowed border-e border-border text-[9px] font-mono uppercase tracking-wider"
                              disabled={!!activeRoom}
                              onClick={(e) => { e.preventDefault(); callUser({ userId: f.id, username: f.username, displayName: f.displayName, avatarUrl: f.avatarUrl ?? null }); }}
                            >
                            ...
                            >
                              {openingDm === f.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <MessageSquare className="w-3 h-3" />}
                              {t("network.chat")}
                            </button>
                            <button
                            ...
                            >
                              <ShieldOff className="w-3 h-3" /> {t("network.block")}
                            </button>
                          </div>
                        )}
                      </div>
