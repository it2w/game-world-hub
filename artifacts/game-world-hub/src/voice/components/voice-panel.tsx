  ...
  return (
    <>
      ...
      {theater && (() => {
        ...
        // Build participants with userId for volume control (null = self)
        const allPeople: Array<{ name: string; avatarUrl: string | null; speaking: boolean; muted: boolean; userId: number | null; isSelf: boolean }> = [
          ...(me ? [{ name: me.displayName, avatarUrl: me.avatarUrl ?? null, speaking: effectiveSpeaking && !effectiveMuted, muted: effectiveMuted, userId: null as null, isSelf: true }] : []),
          ...effectivePeers.map(p => ({ name: p.displayName, avatarUrl: p.avatarUrl ?? null, speaking: p.speaking && !p.muted, muted: p.muted, userId: p.userId as number, isSelf: false })),
        ];
        const nonPresenters = allPeople.filter(p => p.name !== theater.label);
              ...
              <div className="flex items-center gap-5 max-w-5xl mx-auto">
                ...
                <div className="shrink-0 flex flex-col items-center gap-1.5" onClick={e => e.stopPropagation()}>
                  <button
                    ...
                    style={{
                      background: effectiveMuted ? "rgba(239,68,68,0.18)" : "rgba(255,255,255,0.07)",
                      border: effectiveMuted ? "2px solid rgba(239,68,68,0.55)" : "2px solid rgba(255,255,255,0.12)",
                    }}
                  ...
                  <span className="text-[8px] font-semibold uppercase tracking-widest" style={{ color: effectiveMuted ? "rgba(239,68,68,0.8)" : "rgba(255,255,255,0.35)" }}>
