...
export function VoicePanel() {
  ...
  return (
    <>
            ...
            <>
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-[9px] uppercase tracking-[0.18em] leading-none mb-[3px]"
                    style={{ color: stageInfo?.isStageRoom ? "#f59e0b" : undefined }}
                  >
                    {effectiveRoom.kind === "party"
                      ? t("voice.voice")
                      : stageInfo?.isStageRoom
                      ? "STAGE"
                      : t("voice.call")}
                  </div>
              ...
              >
                ...
                {screenSharers.map((p) => (
                  <ScreenThumb
                    ...
                    label={p.displayName}
                    onOpen={() => setTheater({ stream: p.screenStream!, label: p.displayName, avatarUrl: p.avatarUrl ?? null })}
                  />
                ))}
              </div>
            )}
              ...
              >
                {cameraEnabled && localCameraStream && (
                  <ScreenThumb
                    ...
                    self
                    cameraLabel
                    onOpen={() => setTheater({ stream: localCameraStream, label: me?.displayName ?? t("voice.you"), avatarUrl: me?.avatarUrl ?? null })}
                  />
                )}
                {cameraViewers.map((p) => (
                  <ScreenThumb
                    ...
                    label={p.displayName}
                    cameraLabel
                    onOpen={() => setTheater({ stream: p.cameraStream!, label: p.displayName, avatarUrl: p.avatarUrl ?? null })}
                  />
                ))}
              </div>
            )}

            {/* ── Peer list / Stage layout ──────────────────────────────── */}
            {stageInfo?.isStageRoom ? (
              <StageLayout />
            ) : (
              <div className="flex flex-col">
      ...
      {/* ── Theater view ───────────────────────────────────────────────────── */}
      {theater && (() => {
        const presenterPeer = effectivePeers.find(p => p.displayName === theater.label);
        ...
        const peerRows = effectivePeers.filter(p => p.displayName !== theater.label);
        ...
        return (
          ...
          >
            ...
            <div
              className="shrink-0"
              style={{ background: "rgba(6,6,18,0.98)", borderTop: "1px solid rgba(255,255,255,0.07)" }}
              onClick={(e) => e.stopPropagation()}
            >
              {/* row 1: presenter info + screen-audio volume + self mic */}
              <div
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: theater.avatarUrl ? undefined : nameColor(theater.label),
                ...
                <div className="shrink-0">
                  <p className="text-sm font-bold text-white leading-none mb-0.5">{theater.label}</p>
                ...
                {/* screen-audio volume — always visible */}
                <div
                  className="flex items-center gap-1.5 px-3 py-1.5 ms-1"
                ...
                <button
                  onClick={toggleMute}
                  className="flex items-center gap-2 px-3 py-1.5 shrink-0 transition-all duration-200"
                  style={{
                    background: effectiveMuted ? "rgba(239,68,68,0.15)" : "rgba(255,255,255,0.05)",
                    border: effectiveMuted ? "1px solid rgba(239,68,68,0.45)" : "1px solid rgba(255,255,255,0.1)",
                  }}
                  title={effectiveMuted ? t("voice.unmute") : t("voice.mute")}
                >
                  {effectiveMuted
                  ...
                  <span
                    ...
                    style={{ color: effectiveMuted ? "rgba(239,68,68,0.9)" : "rgba(255,255,255,0.4)" }}
                  >
                    {effectiveMuted ? t("voice.muted") : t("voice.mic")}
                  </span>
                </button>
              </div>
                ...
                <div className="flex items-center gap-3 px-5 py-2.5 overflow-x-auto">
                  {peerRows.map(p => {
                    ...
                    return (
                      <div
                        key={p.userId}
                        className="flex items-center gap-2 shrink-0 px-2.5 py-1.5"
                      ...
                      >
                        ...
                        {/* name */}
                        <span
                          className="text-[10px] font-medium truncate max-w-[60px] shrink-0"
